...
describe 'Week3 method' do
  ...
  context '#configure' do 
    ...
    it 'returns default value when :path is missing from parameter' do
      subject.configure({:version => '0.1.0', :mode => 'production'}).should eq ({:path => './', :version => '0.1.0', :mode => 'production'})
    end

    it 'returns overridden value when :version is included in the parameter' do
      subject.configure({:version => 'overridden'}).should eq ({:path => './', :version => 'overridden', :mode => 'production'})
    end  
